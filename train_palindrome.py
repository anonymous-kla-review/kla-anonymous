...
from palindrome_data import PalindromeDataset, generate_palindrome_data
...
def parse_int_csv(values: str) -> list[int]:
    ...
    for token in values.split(","):
        ...
        value = int(token)
        if value < 2:
            raise ValueError(f"Invalid sequence length {value}. Must be >= 2.")
        parsed.append(value)
    return sorted(set(parsed))


def infer_source_seq_len(dataset_total_len: int) -> int:
    if dataset_total_len < 5:
        raise ValueError(f"Unexpected palindrome total length: {dataset_total_len}")
    if dataset_total_len % 2 != 1:
        raise ValueError(
            f"Unexpected palindrome total length {dataset_total_len}. "
            "Expected odd length (2 * seq_len + 1)."
        )
    return (dataset_total_len - 1) // 2
...
def validate_dataset_predict_first_token_setting(dataset, dataset_name: str, expected_predict_first_token: bool) -> None:
    total_len = int(dataset.input_ids.shape[1])
    source_seq_len = infer_source_seq_len(total_len)

    sep_supervision_mask = dataset.labels[:, source_seq_len] != -100
    has_sep_supervision = bool(torch.any(sep_supervision_mask).item())
    all_sep_supervised = bool(torch.all(sep_supervision_mask).item())

    if has_sep_supervision != all_sep_supervised:
        raise ValueError(
            (
                f"{dataset_name}: mixed supervision at separator position. "
                "The dataset appears inconsistent/corrupted. Regenerate cached data."
            )
        )

    if has_sep_supervision != expected_predict_first_token:
        raise ValueError(
            (
                f"{dataset_name}: predict_first_token mismatch. "
                f"Dataset has predict_first_token={has_sep_supervision}, "
                f"but current run expects predict_first_token={expected_predict_first_token}. "
                "Regenerate data or switch the run flag to match the cached dataset."
            )
        )
...
def build_eval_loader(
    ...
    cache_path = os.path.join(
        ...
        (
            ...
            f"_n{num_examples}_v{args.vocab_size}_pf{int(args.predict_first_token)}.npz"
        ),
    )
    ...
    if fabric.global_rank == 0 and not os.path.exists(cache_path):
        ...
        eval_data = generate_palindrome_data(
        ...
        fabric.print(f"Generated palindrome eval data at {cache_path}")

    fabric.barrier()

    val_dataset = PalindromeDataset(data_path=cache_path)
...
def main(args):
    if args.debug:
        wandb_logger = WandbLogger(
            project=os.environ.get("WANDB_PROJECT", "palindrome_linear_attn"),
            mode="disabled",
            name=args.exp_name,
        )
    else:
    ...
    train_dataset = PalindromeDataset(data_path=os.path.join(args.data_dir, "train.npz"))
    val_dataset = PalindromeDataset(data_path=os.path.join(args.data_dir, "val.npz"))
    test_dataset = PalindromeDataset(data_path=os.path.join(args.data_dir, "test.npz"))

    train_source_seq_len = infer_source_seq_len(int(train_dataset.input_ids.shape[1]))

    eval_seq_lens = parse_int_csv(args.eval_seq_lens) if args.eval_seq_lens.strip() else []
    eval_seq_lens.append(train_source_seq_len)
    ...
    required_block_size = 2 * max(eval_seq_lens) + 1
    ...
    validate_dataset_predict_first_token_setting(train_dataset, "train.npz", args.predict_first_token)
    validate_dataset_predict_first_token_setting(val_dataset, "val.npz", args.predict_first_token)
    validate_dataset_predict_first_token_setting(test_dataset, "test.npz", args.predict_first_token)

    if fabric.global_rank == 0:
        fabric.print(
            (
                f"Palindrome eval setup: eval_seq_lens={eval_seq_lens}, "
                f"train_seq_len={train_source_seq_len}, eval_num_val={eval_num_val}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_name", type=str, default="palindrome_exp")
